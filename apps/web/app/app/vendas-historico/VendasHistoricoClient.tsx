  ...
  return (
    <div className="space-y-3">
      <div className="card p-4">
        ...
        <p className="mt-1 text-xs text-muted">Abra o PDF/relatório do sistema antigo, selecione tudo (Ctrl+A), copie (Ctrl+C) e cole abaixo. O sistema entende linhas no formato <code className="rounded bg-surface-2 px-1 py-0.5 text-[11px]">CÓDIGO DATA PRODUTO UN QTDE UNITÁRIO DESCONTO TOTAL</code>. Cabeçalhos/rodapés são ignorados.</p>
        ...
        <div className="mt-2 flex gap-2">
          <button onClick={processar} className="btn-grad">Processar</button>
      ...
      {parsed && parsed.rows.length > 0 && (
        <div className="card p-4">
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {parsed.rows.slice(0, 100).map((r, i) => (
                  <tr key={i} className="border-t border-line hover:bg-surface-2"><td className="px-2 py-1 font-mono">{r.legacyCode}</td><td className="px-2 py-1">{r.saleDate.split("-").reverse().join("/")}</td><td className="px-2 py-1">{r.productName}</td><td className="px-2 py-1 text-right">{r.qty}</td><td className="px-2 py-1 text-right">{brl(r.unitPriceCents)}</td><td className="px-2 py-1 text-right">{brl(r.discountCents)}</td><td className="px-2 py-1 text-right">{brl(r.totalCents)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
          ...
          {parsed.bad.length > 0 && (
            <details className="mt-2 text-xs"><summary className="cursor-pointer text-warn">{parsed.bad.length} linha(s) não reconhecida(s) — verifique se ficou faltando algo</summary>
              <div className="mt-1 max-h-40 overflow-auto rounded-lg bg-surface-2 p-2 font-mono text-[10px] text-muted">{parsed.bad.slice(0, 50).map((b, i) => <div key={i}>{b}</div>)}</div>
            </details>
          )}
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Kpi label="Faturamento" value={brl(g.totalCents ?? 0)} cls="text-success" />
        <Kpi label="Descontos" value={brl(g.descontoCents ?? 0)} cls="text-warn" />
      </div>
      ...
      <div className="card p-4">
        ...
        {batches.length === 0 ? <p className="text-sm text-muted">Nenhum lote importado.</p> : batches.map((b) => (
          <div key={b.batchId} className="flex items-center justify-between border-t border-line py-2 text-sm first:border-t-0">
            <span className="text-muted">{new Date(b.criadoEm).toLocaleString("pt-BR")} · {b.itens} itens · {brl(b.totalCents)}</span>
            <button onClick={() => undo(b.batchId)} className="rounded-xl border border-danger/40 px-3 py-1 text-xs text-danger transition hover:border-danger">Apagar lote</button>
          </div>
        ))}
      </div>
    </div>
...
function Kpi({ label, value, cls }: { label: string; value: any; cls?: string }) {
  return <div className="rounded-2xl border border-line bg-surface px-5 py-4"><p className="text-xs uppercase tracking-wider text-muted">{label}</p><p className={`mt-1 text-2xl font-black ${cls ?? "text-fg"}`}>{value}</p></div>;
}
