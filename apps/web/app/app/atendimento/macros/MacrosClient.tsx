    ...
    <div className="space-y-4">
      {edit ? (
        <section className="card space-y-4 border-brand/40">
          ...
          <input value={edit.name ?? ""} onChange={(e) => setEdit({ ...edit, name: e.target.value })} placeholder="Nome (ex.: Receber pedido)" className="input-base" />
          <textarea value={edit.description ?? ""} onChange={(e) => setEdit({ ...edit, description: e.target.value })} rows={2} placeholder="Descrição (opcional)" className="input-base" />
          ...
          <div className="space-y-2">
            ...
            {(edit.actions ?? []).map((a, i) => (
              <div key={i} className="rounded-xl border border-line bg-surface-2 p-3">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button onClick={() => removeAction(i)} className="text-muted transition hover:text-danger">×</button>
                </div>
                {a.kind === "send_message" && (
                  <>
                    <textarea value={(a as any).body} onChange={(e) => patchAction(i, { body: e.target.value } as any)} rows={2} placeholder="Mensagem... use {{cliente.nome}}, {{operador.nome}}, etc" className="input-base" />
                    <label className="mt-1 flex items-center gap-1 text-[10px] text-muted">
                      <input type="checkbox" checked={(a as any).isPrivate ?? false} onChange={(e) => patchAction(i, { isPrivate: e.target.checked } as any)} className="accent-brand" /> nota interna (cliente não vê)
                    </label>
                  </>
                )}
                {a.kind === "transfer_team" && (
                  <select value={(a as any).teamId ?? ""} onChange={(e) => patchAction(i, { teamId: e.target.value } as any)} className="input-base">
                    <option value="">— escolha equipe —</option>
                    {teams.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                  </select>
                )}
                {(a.kind === "add_label" || a.kind === "remove_label") && (
                  <select value={(a as any).labelId ?? ""} onChange={(e) => patchAction(i, { labelId: e.target.value } as any)} className="input-base">
                    <option value="">— escolha label —</option>
                    {labels.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
                  </select>
                )}
                {a.kind === "set_status" && (
                  <select value={(a as any).status} onChange={(e) => patchAction(i, { status: e.target.value as any })} className="input-base">
                ...
                {a.kind === "set_priority" && (
                  <select value={(a as any).priority} onChange={(e) => patchAction(i, { priority: e.target.value } as any)} className="input-base">
            ...
            <div className="flex flex-wrap gap-1.5">
              {(["send_message", "transfer_team", "add_label", "remove_label", "set_status", "set_priority", "assign"] as const).map((k) => (
                <button key={k} onClick={() => addAction(k)} className="rounded-lg border border-line px-2.5 py-1 text-xs transition hover:border-brand hover:text-brand">+ {KIND_LABEL[k]}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button onClick={save} className="btn-grad px-5">Salvar macro</button>
          </div>
        </section>
      ) : (
        <button onClick={() => setEdit({ name: "", actions: [] })} className="btn-grad px-5">+ Nova macro</button>
      )}

      <section className="space-y-2">
        {items.length === 0
          ? <p className="card text-sm text-muted">Nenhuma macro cadastrada ainda.</p>
          : items.map((m) => (
              <div key={m.id} className="card flex items-center gap-3 p-4">
                ...
                <button onClick={() => setEdit(m)} className="rounded-lg border border-line px-2.5 py-1 text-xs transition hover:border-brand hover:text-brand">editar</button>
                <button onClick={() => remove(m.id)} className="text-muted transition hover:text-danger">×</button>
              </div>
            ))}
      </section>
    </div>
