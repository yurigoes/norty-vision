  ...
  if (forbidden) {
    return (
      <div className="max-w-2xl">
        ...
        <p className="card mt-6 text-sm text-muted">Apenas supervisores (admin da empresa) acessam este painel.</p>
      </div>
    );
  }
      ...
      <section className="card">
        ...
        <div className="space-y-1">
          {presence.length === 0 ? <p className="text-xs text-muted">Nenhum operador.</p> : presence.map((p) => (
            <div key={p.membershipId} className="flex items-center justify-between gap-2 rounded-xl border border-line bg-surface-2 px-3 py-2 text-sm">
              ...
              <span className="flex items-center gap-3 text-xs text-muted">
                ...
                <span className={`rounded-full px-2 py-0.5 font-semibold ${p.activeCount >= p.maxConcurrent ? "bg-danger/15 text-danger" : "bg-line text-fg"}`}>{p.activeCount}/{p.maxConcurrent}</span>
              </span>
            </div>
          ))}
        </div>
