...
const STATUS_CLS: Record<string, string> = { aberto: "bg-brand/15 text-brand", aguardando_master: "bg-warn/15 text-warn", aguardando_usuario: "bg-purple-500/15 text-purple-300", resolvido_ia: "bg-success/15 text-success", resolvido: "bg-success/15 text-success", fechado: "bg-surface-2 text-muted" };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1.5">
        {FILTERS.map(([v, l]) => <button key={v} onClick={() => setFilter(v)} className={`rounded-full px-3 py-1 text-xs transition ${filter === v ? "bg-brand text-white" : "border border-line text-muted hover:border-brand"}`}>{l}</button>)}
      </div>
      {items === null ? <p className="text-sm text-muted">Carregando…</p> : items.length === 0 ? (
        <p className="card p-8 text-center text-muted">Nenhum chamado.</p>
      ) : (
        <div className="space-y-3">
          {items.map((t) => (
            <button key={t.id} onClick={() => setOpenId(t.id)} className="card flex w-full items-center justify-between gap-3 text-left">
...
function MasterThread({ id, onClose, onChanged }: { id: string; onClose: () => void; onChanged: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col rounded-2xl border border-line bg-surface p-5 shadow-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-3 flex-1 space-y-2 overflow-y-auto rounded-xl border border-line/60 bg-surface-2 p-3">
          {(t.messages ?? []).map((m: any) => (
            <div key={m.id} className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${m.author === "master" ? "ml-auto bg-purple-500/15" : m.author === "usuario" ? "bg-brand/15" : m.author === "sistema" ? "bg-surface text-muted" : "bg-surface border border-line"} ${m.internal ? "border border-dashed border-warn/50" : ""}`}>
        ...
        <div className="mt-3 space-y-2">
          <textarea value={reply} onChange={(e) => setReply(e.target.value)} rows={2} placeholder="Responder ao usuário…" className="input-base" />
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex gap-2">
              <button disabled={busy} onClick={() => send(false)} className="rounded-xl border border-line px-4 py-2 text-sm transition hover:border-brand disabled:opacity-50">Responder</button>
              <button disabled={busy || internal} onClick={() => send(true)} className="btn-grad disabled:opacity-50">Responder e resolver</button>
            </div>
          </div>
        </div>
      </div>
