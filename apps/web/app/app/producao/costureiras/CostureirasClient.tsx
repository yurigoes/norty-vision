    ...
    <div className="space-y-5">
      ...
      {selected ? (
        ...
        <section className="grid gap-3 sm:grid-cols-2">
          {suppliers.length === 0
            ? <p className="rounded-lg border border-line bg-bg/60 p-6 text-sm text-muted">Nenhuma costureira cadastrada. Cadastre uma em <a href="/app/fornecedores" className="text-brand hover:underline">Fornecedores</a> selecionando tipo "Costureira".</p>
            : suppliers.map((s) => (
                <button key={s.id} onClick={() => setSelected(s)} className="rounded-xl border border-line bg-bg/60 p-4 text-left transition hover:border-brand">
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-line bg-bg/60 p-4">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button onClick={() => { const t = new Date(); setTo(isoDate(t)); setFrom(isoDate(new Date(t.getTime() - 7 * 86400_000))); }} className="rounded-full border border-line px-3 py-1 text-xs">7d</button>
          <button onClick={() => { const t = new Date(); setTo(isoDate(t)); setFrom(isoDate(new Date(t.getTime() - 30 * 86400_000))); }} className="rounded-full border border-line px-3 py-1 text-xs">30d</button>
          <label className="flex flex-col text-[10px] uppercase text-muted">de
            <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="rounded border border-line bg-bg/40 px-2 py-1 text-sm" />
          </label>
          <label className="flex flex-col text-[10px] uppercase text-muted">até
            <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="rounded border border-line bg-bg/40 px-2 py-1 text-sm" />
          </label>
        </div>
        ...
        <div className="mt-3 space-y-1">
          {(report?.items ?? []).map((o) => (
            <div key={o.id} className="flex items-center justify-between rounded border border-line/60 bg-bg/40 px-3 py-2 text-xs">
      ...
      <section className="rounded-xl border border-line bg-bg/60 p-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          {selectedPending.size > 0 && (
            <button onClick={() => setPayOpen(true)} className="rounded-lg bg-brand px-3 py-1.5 text-xs font-semibold text-white">
        ...
        {pending.length === 0 ? (
          ...
          <div className="space-y-1">
            {pending.map((p) => (
              <label key={p.id} className="flex items-center gap-3 rounded border border-line/60 bg-bg/40 px-3 py-2 text-xs hover:border-brand">
                <input
                  type="checkbox"
...
function PayoutModal({ supplierId, items, onClose, onPaid }: { supplierId: string; items: PendingItem[]; onClose: () => void; onPaid: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-line bg-bg p-5">
        ...
        <label className="mt-4 block">
          ...
          <select value={method} onChange={(e) => setMethod(e.target.value)} className="w-full rounded border border-line bg-bg/60 px-2 py-2 text-sm">
        ...
        <label className="mt-3 block">
          ...
          <input
            ...
            className="w-full rounded border border-line bg-bg/60 px-2 py-2 text-sm"
          />
        ...
        <div className="mt-5 flex gap-2">
          <button onClick={onClose} disabled={busy} className="flex-1 rounded-lg border border-line py-2 text-sm">Cancelar</button>
          <button onClick={pay} disabled={busy} className="flex-1 rounded-lg bg-brand py-2 text-sm font-semibold text-white disabled:opacity-50">{busy ? "Pagando…" : "Confirmar pagamento"}</button>
        </div>
      </div>
    </div>
  );
...
function UnassignedPanel({ suppliers, orders, onAssigned, toast }: { suppliers: Supplier[]; orders: any[]; onAssigned: () => void; toast: (m: string, t: "success" | "error") => void }) {
  ...
  if (orders.length === 0) return <p className="rounded-lg border border-line bg-bg/60 p-6 text-sm text-muted">Nenhuma OS sem costureira no momento.</p>;
  return (
    <section className="space-y-2">
      {orders.map((o) => (
        <div key={o.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-line bg-bg/60 p-3">
