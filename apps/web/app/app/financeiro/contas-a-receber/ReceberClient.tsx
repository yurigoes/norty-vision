  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => setCashflowOpen(true)} className="ml-auto rounded-xl border border-line bg-surface px-3 py-2 text-sm font-medium transition hover:border-brand">Fluxo de caixa</button>
        <a href={`/api/receivables/report.pdf?status=${tab}`} target="_blank" rel="noreferrer" className="rounded-xl border border-line bg-surface px-3 py-2 text-sm font-medium transition hover:border-brand">PDF</a>
        <a href={`/api/receivables/export?status=${tab}`} className="rounded-xl border border-line bg-surface px-3 py-2 text-sm font-medium transition hover:border-brand">CSV</a>
        <button onClick={() => setCreating(true)} className="btn-grad px-4 py-2">+ Novo título</button>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-line bg-bg p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2">
              {rows.map((r, i) => (
                <div key={i} className="flex flex-wrap gap-2">
                  ...
                  <input value={r.value} onChange={(e) => setRows((a) => a.map((x, idx) => idx === i ? { ...x, value: e.target.value } : x))} inputMode="decimal" placeholder="R$ valor" className="input-base w-32 px-2" />
        ...
        <div className="mt-4 flex gap-2">
          <button disabled={busy} onClick={save} className="btn-grad flex-1 py-2">{busy ? "Salvando…" : "Lançar título"}</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl border border-line bg-bg p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <label className="mt-3 block"><span className="mb-1 block text-[10px] uppercase text-muted">Valor recebido (R$)</span><input value={paid} onChange={(e) => setPaid(e.target.value)} inputMode="decimal" className="input-base" /></label>
