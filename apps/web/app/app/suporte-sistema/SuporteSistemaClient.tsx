  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setCreating(true)} className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white hover:opacity-90">+ Novo chamado</button>
      </div>
      {items === null ? <p className="text-sm text-muted">Carregando…</p> : items.length === 0 ? (
        <p className="rounded-xl border border-line bg-bg/60 p-8 text-center text-muted">Nenhum chamado ainda.</p>
      ) : (
      ...
      <div className="w-full max-w-lg rounded-2xl border border-line bg-bg p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-3 space-y-3">
          <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Tipo</span>
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm">
              {Object.entries(CAT_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
          </label>
          <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Assunto</span><input value={subject} onChange={(e) => setSubject(e.target.value)} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" /></label>
          <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Descrição</span><textarea value={body} onChange={(e) => setBody(e.target.value)} rows={4} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" placeholder={SECURE.has(category) ? "Ex.: quero trocar minha senha" : "Descreva sua dúvida ou problema"} /></label>
        </div>
        <div className="mt-4 flex gap-2">
          <button disabled={busy} onClick={save} className="flex-1 rounded-lg bg-brand py-2 text-sm font-semibold text-white disabled:opacity-50">{busy ? "Abrindo…" : "Abrir chamado"}</button>
      ...
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col rounded-2xl border border-line bg-bg p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-3 flex-1 space-y-2 overflow-y-auto rounded-lg border border-line/60 bg-bg/40 p-3">
          {(t.messages ?? []).map((m: any) => (
            <div key={m.id} className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${m.author === "usuario" ? "ml-auto bg-brand/15" : m.author === "master" ? "bg-purple-500/15" : m.author === "sistema" ? "bg-line/40 text-muted" : "bg-bg/70 border border-line"}`}>
        ...
        {secure ? (
          ...
          <div className="mt-3 flex gap-2">
            <input value={reply} onChange={(e) => setReply(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") send(); }} placeholder="Responder…" className="flex-1 rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
            <button disabled={busy} onClick={send} className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Enviar</button>
          </div>
        ) : (
          <div className="mt-3 flex gap-2">
            <input value={reply} onChange={(e) => setReply(e.target.value)} placeholder="Reabrir com uma nova mensagem…" className="flex-1 rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
            <button disabled={busy} onClick={send} className="rounded-lg border border-line px-4 py-2 text-sm hover:border-brand disabled:opacity-50">Enviar</button>
          </div>
        )}
      </div>
    </div>
...
function SecureWizard({ ticket, onChanged }: { ticket: any; onChanged: () => void }) {
  ...
  if (done) return <p className="mt-3 rounded-lg border border-green-500/30 bg-green-500/10 p-3 text-center text-sm text-green-300">✅ Concluído.</p>;
  ...
  return (
    <div className="mt-3 rounded-lg border border-amber-500/30 bg-amber-500/5 p-3">
      <p className="text-sm font-medium text-amber-200">🔒 Assistente seguro — {CAT_LABEL[ticket.category]}</p>
      ...
      {mode === "self" && (
        <div className="mt-2 space-y-2">
          <input type="password" value={curPw} onChange={(e) => setCurPw(e.target.value)} placeholder="Senha atual" className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
          <input type="password" value={newPw} onChange={(e) => setNewPw(e.target.value)} placeholder="Nova senha (mín. 8)" className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
          <p className="text-[11px] text-muted">Por segurança, a senha não fica registrada no chamado.</p>
          <div className="flex gap-2">
            <button onClick={() => setMode("menu")} className="rounded-lg border border-line px-3 py-2 text-xs text-muted hover:text-fg">voltar</button>
            <button disabled={busy} onClick={doSelf} className="flex-1 rounded-lg bg-brand px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Trocar senha</button>
          </div>
        </div>
      )}
        ...
        <div className="mt-2 space-y-2">
          {!reqId ? (
            <>
              ...
              <div className="flex gap-2">
                <button onClick={() => setMode("menu")} className="rounded-lg border border-line px-3 py-2 text-xs text-muted hover:text-fg">voltar</button>
                <button disabled={busy} onClick={sendOtp} className="flex-1 rounded-lg bg-brand px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Enviar código</button>
              </div>
            </>
          ) : (
            <>
              <input value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 5))} inputMode="numeric" placeholder="Código (5 dígitos)" className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-center font-mono text-lg tracking-[0.4em]" />
              <input type={newType} value={newVal} onChange={(e) => setNewVal(e.target.value)} placeholder={newLabel} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
              <div className="flex gap-2">
                <button onClick={sendOtp} disabled={busy} className="rounded-lg border border-line px-3 py-2 text-xs text-muted hover:text-fg">reenviar</button>
                <button disabled={busy} onClick={applyOtp} className="flex-1 rounded-lg bg-brand px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Confirmar</button>
              </div>
            </>
          )}
        </div>
