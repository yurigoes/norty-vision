    ...
    <main className="max-w-4xl">
      ...
      <section className="mb-5 rounded-xl border border-line bg-bg/60 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Regime (CRT)</span>
            <select value={c.crt} onChange={(e) => set("crt", Number(e.target.value))} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm"><option value={1}>Simples Nacional</option><option value={2}>Simples (excesso)</option><option value={3}>Regime Normal</option></select></label>
      ...
      <section className="mb-5 rounded-xl border border-line bg-bg/60 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Ambiente</span>
            <select value={c.ambiente} onChange={(e) => set("ambiente", Number(e.target.value))} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm"><option value={2}>Homologação (teste)</option><option value={1}>Produção</option></select></label>
          ...
          <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Token CSC {c.cscSet ? "(definido — vazio mantém)" : ""}</span><input type="password" value={c.cscToken ?? ""} onChange={(e) => set("cscToken", e.target.value)} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" /></label>
        </div>
      ...
      <section className="mb-5 rounded-xl border border-line bg-bg/60 p-5">
        ...
        <div className="mt-3 flex flex-wrap items-end gap-2">
          ...
          <label className="cursor-pointer rounded-lg border border-line px-3 py-2 text-sm hover:border-brand">{busy ? "Validando…" : c.a1?.configured ? "Trocar .pfx" : "Subir .pfx"}<input type="file" accept=".pfx,.p12,application/x-pkcs12" className="hidden" onChange={onCert} /></label>
        </div>
      </section>

      <button onClick={save} className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white">Salvar configuração</button>
    ...
    <section className="mt-8 rounded-xl border border-brand/30 bg-brand/5 p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <label className="block sm:col-span-2">
          <span className="mb-1 block text-[10px] uppercase text-muted">Ambiente da NFS-e (independe da NFC-e)</span>
          <select value={cfg.nfseAmbiente == null ? "" : String(cfg.nfseAmbiente)} onChange={(e) => setC("nfseAmbiente", e.target.value === "" ? null : Number(e.target.value))} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm">
        ...
        <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Optante Simples Nacional</span>
          <select value={String(cfg.nfseOpSimpNac ?? 1)} onChange={(e) => setC("nfseOpSimpNac", e.target.value)} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm">
            <option value="1">1 — Não optante</option><option value="2">2 — Optante MEI</option><option value="3">3 — Optante (ME/EPP)</option>
          </select>
        </label>
        <div className="relative block">
          <span className="mb-1 block text-[10px] uppercase text-muted">Código de tributação nacional (6 díg)</span>
          <input value={cfg.nfseCodServico ?? ""} onChange={(e) => searchServ(e.target.value)} onBlur={() => setTimeout(() => setServSug([]), 150)} placeholder="ex.: 130501 (6 díg) ou busque por 'confecção'" className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
          {servSug.length > 0 && (
            <div className="absolute z-20 mt-1 max-h-52 w-full overflow-y-auto rounded-lg border border-line bg-bg shadow-xl">
              {servSug.map((s) => (
                <button type="button" key={s.codigo} onMouseDown={(e) => { e.preventDefault(); setC("nfseCodServico", s.codigo); setServSug([]); }} className="block w-full px-3 py-1.5 text-left text-xs hover:bg-line"><span className="font-mono text-brand">{s.codigo}</span> — {s.descricao}</button>
              ))}
            </div>
          )}
        ...
        <label className="block sm:col-span-2">
          <span className="mb-1 block text-[10px] uppercase text-muted">URL base da API (produção restrita) — opcional</span>
          <input value={cfg.nfseUrlHom ?? ""} onChange={(e) => setC("nfseUrlHom", e.target.value)} placeholder={cfg.defaultHom ?? "https://adn.producaorestrita.nfse.gov.br/contribuintes"} className="w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
      ...
      <button onClick={saveCfg} disabled={busy} className="mt-3 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Salvar NFS-e</button>
      ...
      <div className="mt-5 rounded-lg border border-line bg-bg/40 p-4">
        ...
        <button onClick={emitirTeste} disabled={busy} className="mt-3 rounded-lg border border-brand px-4 py-2 text-sm font-semibold text-brand hover:bg-brand/10 disabled:opacity-50">{busy ? "Emitindo…" : "Emitir DPS de teste"}</button>
        ...
        <div className="mt-4">
          ...
          <div className="space-y-1">
            {list.slice(0, 10).map((o) => (
              <div key={o.id} className="flex items-center justify-between rounded-lg border border-line bg-bg/60 px-3 py-1.5 text-xs">
