  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {tab === "camp" && <button onClick={() => setCreating(true)} className="btn-grad">+ Campanha</button>}
      </div>
      ...
      {tab === "cnpj" ? <CnpjBase isMaster={isMaster} /> : tab === "optout" ? <Optout /> : items === null ? <p className="text-sm text-muted">Carregando…</p> : items.length === 0 ? (
        <p className="rounded-xl border border-line bg-surface p-8 text-center text-muted">Nenhuma campanha. Crie uma com nicho + cidade.</p>
      ) : (
        <div className="space-y-2">
          {items.map((c) => (
            <div key={c.id} className="rounded-xl border border-line bg-surface p-4 transition hover:border-brand/50">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => run(c)} className="btn-grad px-3 py-1.5 text-xs">Rodar agora</button>
                  <button onClick={() => setResultsOf(c)} className="rounded-lg border border-line px-3 py-1.5 text-xs font-medium transition hover:border-brand">Resultados</button>
                  <button onClick={() => del(c)} className="rounded-lg border border-line px-3 py-1.5 text-xs font-medium text-danger transition hover:border-danger">Excluir</button>
                </div>
              </div>
            </div>
          ))}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-2xl border border-line bg-surface p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-3 space-y-3">
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder={src === "cnpj" ? "Nome (ex.: Óticas Feira CNAE 4774-1)" : "Nome (ex.: Óticas Feira de Santana)"} className="input-base" />
          {src === "osm" ? (
            <>
              <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Nicho</span>
                <select value={preset} onChange={(e) => setPreset(Number(e.target.value))} className="input-base">
              ...
              {preset === -1 && (
                <div className="grid grid-cols-2 gap-2">
                  <input value={customK} onChange={(e) => setCustomK(e.target.value)} placeholder="tag OSM (ex.: shop)" className="input-base" />
                  <input value={customV} onChange={(e) => setCustomV(e.target.value)} placeholder="valor (ex.: jewelry)" className="input-base" />
                </div>
              )}
            </>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              <input value={cnae} onChange={(e) => setCnae(e.target.value)} placeholder="CNAE (ex.: 4774100 ou 4774)" className="input-base" />
              <input value={uf} onChange={(e) => setUf(e.target.value)} placeholder="UF (ex.: BA)" maxLength={2} className="input-base" />
            </div>
          )}
          <input value={city} onChange={(e) => setCity(e.target.value)} placeholder={src === "cnpj" ? "Município (ex.: Feira de Santana)" : "Cidade (ex.: Feira de Santana)"} className="input-base" />
          <div className="grid grid-cols-2 gap-2">
            <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Frequência</span>
              <select value={freq} onChange={(e) => setFreq(e.target.value)} className="input-base">{Object.entries(FREQ).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select>
            </label>
            <label className="block"><span className="mb-1 block text-[10px] uppercase text-muted">Limite/rodada</span><input value={limit} onChange={(e) => setLimit(e.target.value.replace(/\D/g, ""))} className="input-base" /></label>
          </div>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={auto} onChange={(e) => setAuto(e.target.checked)} className="h-4 w-4 accent-brand" /> Criar lead automaticamente (vai pra fila de Leads novos)</label>
          <label className="flex items-start gap-2 text-sm"><input type="checkbox" checked={enrich} onChange={(e) => setEnrich(e.target.checked)} className="mt-0.5 h-4 w-4 accent-brand" /> <span>Enriquecer por CNPJ (BrasilAPI) <span className="block text-[11px] text-muted">Quando o registro tiver CNPJ, busca razão social, CNAE e situação; descarta empresas baixadas. {src === "osm" ? "Poucos registros do OSM têm CNPJ." : "Recomendado pra base CNPJ."}</span></span></label>
        </div>
        <div className="mt-4 flex gap-2">
          <button disabled={busy} onClick={save} className="btn-grad flex-1">{busy ? "Salvando…" : "Criar campanha"}</button>
...
function Results({ campaign, onClose }: { campaign: any; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="flex max-h-[85vh] w-full max-w-2xl flex-col rounded-2xl border border-line bg-surface p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto">
          {items === null ? <p className="text-sm text-muted">Carregando…</p> : items.length === 0 ? <p className="text-sm text-muted">Sem resultados ainda. Clique em "Rodar agora".</p> : items.map((r) => {
            const statusCls = r.status === "virou_lead" ? "bg-success/15 text-success" : r.status === "descartado" ? "bg-danger/15 text-danger" : "bg-line text-muted";
            const ativa = !r.situacao || r.situacao.toUpperCase() === "ATIVA";
            return (
              <div key={r.id} className="rounded-lg border border-line bg-surface-2 p-3 text-sm">
                <div className="flex items-center justify-between gap-2">
                  <p className="font-medium">{r.name}</p>
                  <div className="flex items-center gap-1.5">
                    {r.situacao && <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${ativa ? "bg-success/15 text-success" : "bg-danger/15 text-danger"}`}>{r.situacao}</span>}
                ...
                {hasCnpj(r) && (
                  <div className="mt-1.5">
                    <button disabled={busyId === r.id} onClick={() => enrich(r)} className="rounded-lg border border-line px-2.5 py-1 text-[11px] font-medium transition hover:border-brand disabled:opacity-50">{busyId === r.id ? "consultando…" : r.enrichedAt ? "🔄 reconsultar CNPJ" : "🔎 enriquecer por CNPJ"}</button>
                  </div>
                )}
              </div>
            );
    ...
    <div className="space-y-3">
      ...
      {isMaster ? (
        <div className="rounded-xl border border-line bg-surface p-4">
          ...
          <textarea value={csv} onChange={(e) => setCsv(e.target.value)} rows={6} placeholder="40029474000180;Ótica Exemplo;Ótica X;4774100;BA;Feira de Santana;7530000000;contato@otica.com;ATIVA" className="input-base mt-2 font-mono text-xs" />
          <button disabled={busy} onClick={importar} className="btn-grad mt-2">{busy ? "Importando…" : "Importar"}</button>
        </div>
      ) : (
        <p className="rounded-lg border border-warn/30 bg-warn/10 p-3 text-xs text-warn">A base CNPJ é carregada pelo suporte do sistema (master). Para grandes volumes, a carga é feita por script na VPS a partir dos arquivos públicos da Receita.</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-line bg-surface p-4">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        <input value={cnpj} onChange={(e) => setCnpj(e.target.value)} placeholder="00.000.000/0000-00" className="input-base flex-1 min-w-[200px] w-auto" />
        <button disabled={busy} onClick={() => lookup(false)} className="rounded-xl border border-line px-4 py-2 text-sm font-semibold transition hover:border-brand disabled:opacity-50">{busy ? "…" : "Consultar"}</button>
        <button disabled={busy} onClick={() => lookup(true)} className="btn-grad disabled:opacity-50">Consultar + criar lead</button>
      </div>
      {c && (
        <div className="mt-3 rounded-lg border border-line bg-surface-2 p-3 text-sm">
          <div className="flex items-center justify-between gap-2">
            ...
            {c.situacao && <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${res.active ? "bg-success/15 text-success" : "bg-danger/15 text-danger"}`}>{c.situacao}</span>}
          </div>
          ...
          {res.leadId && <p className="mt-1 text-[11px] text-success">✓ Lead na fila de Leads novos</p>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        <input value={val} onChange={(e) => setVal(e.target.value)} placeholder="Telefone (ou CNPJ)" className="input-base flex-1 w-auto" />
