    ...
    <div className="max-w-3xl">
      <header className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold">Maiores dúvidas</h1>
          <p className="text-sm text-muted">O que os clientes mais perguntam — vai aprendendo com o volume de conversas.</p>
        </div>
      ...
      <p className="mb-4 rounded-xl border border-line bg-bg/60 p-4 text-sm">Perguntas classificadas no período: <strong>{data?.total ?? 0}</strong></p>

      <div className="space-y-3">
        {(data?.topics.length ?? 0) === 0 ? (
          <p className="rounded-xl border border-line bg-bg/60 p-8 text-center text-sm text-muted">Sem dados ainda. Conforme as conversas chegam, os temas aparecem aqui.</p>
        ) : data!.topics.map((t) => (
          <div key={t.topic} className="rounded-xl border border-line bg-bg/60 p-4">
            <div className="flex items-center justify-between">
              <span className="font-medium">{t.topic}</span>
              <span className="text-sm text-muted">{t.count}</span>
            </div>
            <div className="mt-1 h-2 rounded-full bg-line"><div className="h-2 rounded-full bg-brand" style={{ width: `${(t.count / max) * 100}%` }} /></div>
            {t.samples.length > 0 && (
            ...
            <div className="mt-2 flex gap-2 text-xs">
              <button onClick={() => suggest(t)} disabled={busy} className="rounded-lg border border-brand/50 px-3 py-1 text-brand hover:bg-brand/10 disabled:opacity-50">✨ Sugerir resposta (IA)</button>
              <button onClick={() => startManual(t)} className="rounded-lg border border-line px-3 py-1 hover:border-brand">criar resposta</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {draft && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setDraft(null)}>
          <div className="w-full max-w-md rounded-2xl border border-line bg-bg p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <input value={draft.shortcut} onChange={(e) => setDraft({ ...draft, shortcut: e.target.value })} className="mt-1 w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
            <label className="mt-2 block text-[10px] uppercase text-muted">Texto {busy && "· gerando com IA…"}</label>
            <textarea value={draft.body} onChange={(e) => setDraft({ ...draft, body: e.target.value })} rows={4} className="mt-1 w-full rounded-lg border border-line bg-bg/40 px-3 py-2 text-sm" />
            <div className="mt-3 flex gap-2">
              <button onClick={saveCanned} className="flex-1 rounded-lg bg-brand py-2 text-sm font-semibold text-white">Salvar resposta</button>
              <button onClick={() => setDraft(null)} className="rounded-lg border border-line px-4 py-2 text-sm text-muted hover:text-fg">cancelar</button>
            </div>
          </div>
        </div>
      )}
