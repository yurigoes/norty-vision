...
import { loginPathFor, readOrgSlug, rememberOrgSlug, safeNext } from "../../lib/orgMemory";
...
export function LoginForm() {
  ...
  // empresa lembrada neste aparelho — atalho de volta pra porta dela
  const [rememberedSlug, setRememberedSlug] = useState<string | null>(null);
  useEffect(() => {
    setCompanySlug(orgSlugFromHost());
    setRememberedSlug(readOrgSlug());
  }, []);
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // empresa do subdomínio vira a "empresa deste aparelho" (sair volta pra ela)
      if (!useMaster && companySlug) rememberOrgSlug(companySlug);
      // ?next= leva de volta pra página que o usuário tentou abrir
      const next = typeof window !== "undefined"
        ? safeNext(new URLSearchParams(window.location.search).get("next"))
        : null;
      window.location.assign(useMaster ? "/app/platform" : (next ?? "/app"));
    } catch {
    ...
    <div className="grid min-h-screen grid-cols-[1.05fr_0.95fr] max-md:grid-cols-1">
      ...
      <main className="grid place-items-center bg-bg p-6 md:p-10">
        <div className="w-full max-w-sm">
          ...
          {rememberedSlug && !companySlug && (
            <a
              href={loginPathFor("equipe", rememberedSlug)}
              className="mb-5 flex items-center gap-2 rounded-xl border border-line bg-surface px-3.5 py-2.5 text-sm text-muted transition hover:border-brand/50 hover:text-fg"
            >
              <span aria-hidden>←</span>
              <span className="min-w-0 truncate">
                Voltar para o acesso da <strong className="text-fg">{rememberedSlug}</strong>
              </span>
            </a>
          )}
